      ...
      for (i4 = Math.max(m, Math.max(localObject[3], Math.max(localObject[0], Math.max(localObject[1], localObject[2]))) + Math.max(arrayOfInt[3], Math.max(arrayOfInt[0], Math.max(arrayOfInt[1], arrayOfInt[2]))));; i4 = m)
      {
        ...
        if ((i3 != 0) || ((i5 != 0) && (f1 > 0.0F)))
        {
          ...
          if (i3 < i10)
          {
            ...
            if ((width != 0) || (i12 != 1073741824))
            {
              ...
              i = ViewUtils.combineMeasuredStates(i, ViewCompat.getMeasuredState(localView) & 0xFF000000);
              f1 -= f2;
              i2 = i1 - i4;
              i1 = i;
              i = i2;
        ...
        for (;;)
        {
          ...
          for (;;)
          {
            ...
            for (;;)
            {
              setMeasuredDimension(0xFF000000 & j | i8, ViewCompat.resolveSizeAndState(Math.max(i + (getPaddingTop() + getPaddingBottom()), getSuggestedMinimumHeight()), paramInt2, j << 16));
              if (n != 0) {
    ...
    for (;;)
    {
      if (i6 < i9)
      {
        ...
        else
        {
          ...
          k = Math.max(k, i8);
          i3 = ViewUtils.combineMeasuredStates(j, ViewCompat.getMeasuredState(localView));
          if ((m != 0) && (width == -1))
          {
            j = 1;
            label571:
    ...
    for (;;)
    {
      ...
      for (;;)
      {
        ...
        for (;;)
        {
          ...
          for (;;)
          {
            setMeasuredDimension(ViewCompat.resolveSizeAndState(Math.max(i + (getPaddingLeft() + getPaddingRight()), getSuggestedMinimumWidth()), paramInt1, m), i6);
            if (n != 0) {
