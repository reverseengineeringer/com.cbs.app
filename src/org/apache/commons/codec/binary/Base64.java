  ...
  extends BaseNCodec
{
  ...
  void decode(byte[] paramArrayOfByte, int paramInt1, int paramInt2, BaseNCodec.Context paramContext)
  {
    ...
    for (;;)
    {
      ...
      if (i < paramInt2)
      {
        arrayOfByte = ensureBufferSize(decodeSize, paramContext);
      ...
      else
      {
        ...
        paramArrayOfByte = ensureBufferSize(decodeSize, paramContext);
      }
      switch (modulus)
      {
      case 1: 
      default: 
        ...
        if ((j >= 0) && (j < DECODE_TABLE.length))
        {
          ...
          {
            modulus = ((modulus + 1) % 4);
            ibitWorkArea = (j + (ibitWorkArea << 6));
            if (modulus == 0)
            {
              j = pos;
              pos = (j + 1);
              arrayOfByte[j] = ((byte)(ibitWorkArea >> 16 & 0xFF));
              j = pos;
              pos = (j + 1);
              arrayOfByte[j] = ((byte)(ibitWorkArea >> 8 & 0xFF));
              j = pos;
              pos = (j + 1);
              arrayOfByte[j] = ((byte)(ibitWorkArea & 0xFF));
            }
          }
        }
        i += 1;
    ...
    paramInt1 = pos;
    pos = (paramInt1 + 1);
    paramArrayOfByte[paramInt1] = ((byte)(ibitWorkArea & 0xFF));
    return;
    ibitWorkArea >>= 2;
    paramInt1 = pos;
    pos = (paramInt1 + 1);
    paramArrayOfByte[paramInt1] = ((byte)(ibitWorkArea >> 8 & 0xFF));
    paramInt1 = pos;
    pos = (paramInt1 + 1);
    paramArrayOfByte[paramInt1] = ((byte)(ibitWorkArea & 0xFF));
  }
  ...
  void encode(byte[] paramArrayOfByte, int paramInt1, int paramInt2, BaseNCodec.Context paramContext)
  {
    ...
    for (;;)
    {
      ...
      if (paramInt2 < 0)
      {
        ...
        {
          paramArrayOfByte = ensureBufferSize(encodeSize, paramContext);
          paramInt1 = pos;
          switch (modulus)
          {
          default: 
          ...
          case 1: 
            paramInt2 = pos;
            pos = (paramInt2 + 1);
            paramArrayOfByte[paramInt2] = encodeTable[(ibitWorkArea >> 2 & 0x3F)];
            paramInt2 = pos;
            pos = (paramInt2 + 1);
            paramArrayOfByte[paramInt2] = encodeTable[(ibitWorkArea << 4 & 0x3F)];
          ...
          for (;;)
          {
            ...
            System.arraycopy(lineSeparator, 0, paramArrayOfByte, pos, lineSeparator.length);
            pos += lineSeparator.length;
            return;
            paramInt2 = pos;
            pos = (paramInt2 + 1);
            paramArrayOfByte[paramInt2] = encodeTable[(ibitWorkArea >> 10 & 0x3F)];
            paramInt2 = pos;
            pos = (paramInt2 + 1);
            paramArrayOfByte[paramInt2] = encodeTable[(ibitWorkArea >> 4 & 0x3F)];
            paramInt2 = pos;
      ...
      else
      {
        ...
        while (i < paramInt2)
        {
          byte[] arrayOfByte = ensureBufferSize(encodeSize, paramContext);
          modulus = ((modulus + 1) % 3);
          ...
          ibitWorkArea = (j + (ibitWorkArea << 8));
          if (modulus == 0)
          {
            j = pos;
            pos = (j + 1);
            arrayOfByte[j] = encodeTable[(ibitWorkArea >> 18 & 0x3F)];
            j = pos;
            pos = (j + 1);
            arrayOfByte[j] = encodeTable[(ibitWorkArea >> 12 & 0x3F)];
            j = pos;
            pos = (j + 1);
            arrayOfByte[j] = encodeTable[(ibitWorkArea >> 6 & 0x3F)];
            j = pos;
            pos = (j + 1);
            arrayOfByte[j] = encodeTable[(ibitWorkArea & 0x3F)];
            currentLinePos += 4;
            if ((lineLength > 0) && (lineLength <= currentLinePos))
            {
              System.arraycopy(lineSeparator, 0, arrayOfByte, pos, lineSeparator.length);
              pos += lineSeparator.length;
              currentLinePos = 0;
            }
          }
